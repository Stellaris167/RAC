...
#
# Usage:
#   EXP=baseline  bash run_internvl3_8b_grpo.sh
#   EXP=ranking   bash run_internvl3_8b_grpo.sh
#   EXP=pair_T0.2 bash run_internvl3_8b_grpo.sh
# =============================================================================
set -xeuo pipefail
...
HYDRA_FULL_ERROR=1 "$PYTHON_BIN" -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=internvl3-8b-${ALGO}-${EXP} \
    trainer.logger="['console','wandb']" \
    trainer.n_gpus_per_node=${N_GPUS_PER_NODE:-6} \
    trainer.nnodes=${NNODES:-1} \
    trainer.validation_data_dir=$PROJECT_DIR/logs/val_dumps/internvl3-8b-${ALGO}-${EXP} \
    trainer.default_local_dir=$PROJECT_DIR/checkpoints/internvl3-8b-${ALGO}-${EXP}
