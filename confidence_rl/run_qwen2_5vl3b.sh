...
#
# Usage:
#   EXP=baseline  bash run_qwen2_5vl3b_grpo.sh
#   EXP=ranking   bash run_qwen2_5vl3b_grpo.sh
#   EXP=pair_T0.2 bash run_qwen2_5vl3b_grpo.sh
# =============================================================================
set -xeuo pipefail
...
MODEL=${MODEL:-/home/yby/models/Qwen2.5-VL-3B-Instruct}
ALGO=${ALGO:-grpo}
...
HYDRA_FULL_ERROR=1 "$PYTHON_BIN" -m verl.trainer.main_ppo \
    ...
    data.prompt_key=message_qwenvl \
    ...
    trainer.experiment_name=qwen2.5vl3b-${ALGO}-${EXP} \
    trainer.logger="['console','wandb']" \
    trainer.n_gpus_per_node=8 \
    trainer.nnodes=3 \
    trainer.validation_data_dir=$PROJECT_DIR/logs/val_dumps/qwen2.5vl3b-${ALGO}-${EXP} \
    trainer.default_local_dir=$PROJECT_DIR/checkpoints/qwen2.5vl3b-${ALGO}-${EXP}
